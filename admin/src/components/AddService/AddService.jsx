...
import { Calendar, Clock, Trash2, Plus, Upload, X, AlertCircle, Check } from "lucide-react";
...
export default function AddService({ serviceId: serviceIdProp, onSuccess }) {
  ...
  const serviceId = serviceIdProp || paramId;
  ...
  useEffect(() => {
    ...
    async function loadService() {
      ...
      try {
        ...
        // normalize slots if map
        let normalizedSlots = [];
        if (data.slots && typeof data.slots === "object" && !Array.isArray(data.slots)) {
          Object.keys(data.slots).forEach(dateStr => {
            const timeList = data.slots[dateStr];
            if (Array.isArray(timeList)) {
              // dateStr is YYYY-MM-DD
              const dateParts = dateStr.split("-");
              if (dateParts.length === 3) {
                const year = dateParts[0];
                const monIdx = Number(dateParts[1]) - 1;
                const day = String(Number(dateParts[2])).padStart(2, "0");
                const monShort = months[monIdx];
                timeList.forEach(t => {
                  normalizedSlots.push(`${day} ${monShort} ${year} • ${t}`);
                });
              }
            }
          });
        } else if (Array.isArray(data.slots)) {
          normalizedSlots = data.slots;
        }
  ...
  return (
    <div className={addServiceStyles.container.main}>
      <form onSubmit={handleSubmit} className={addServiceStyles.container.form}>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 border-b border-emerald-50 pb-5">
          <div>
            ...
            <p className={addServiceStyles.header.subtitle}>
              {serviceId ? "Modify details and schedule slots for this service" : "Create a new medical service with booking availability"}
            </p>
          </div>
          <div className={addServiceStyles.headerActions}>
            ...
            >
              {submitting ? "Saving..." : serviceId ? "Update Service" : "Add Service"}
            </button>
          </div>
        </div>
        ...
        <div className={addServiceStyles.grids.main}>
          <div className="col-span-1">
            <label className={addServiceStyles.labels.standard + " mb-2 block"}>
              Service Image
            </label>
        ...
        <div className={addServiceStyles.toast.container}>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <h4 className={addServiceStyles.toast.title}>{toast.title}</h4>
              <p className={addServiceStyles.toast.message}>{toast.message}</p>
            </div>
            <button
