...
import { Search, Calendar, BadgeIndianRupee, AlertCircle, Loader2, Clock, CheckCircle, XCircle, User, Phone } from "lucide-react";
    ...
    <div className={serviceAppointmentsStyles.container}>
      <div className={serviceAppointmentsStyles.headerContainer}>
        <div className={serviceAppointmentsStyles.headerTitleContainer}>
          <h2 className={serviceAppointmentsStyles.headerTitle}>Service Bookings</h2>
          <p className={serviceAppointmentsStyles.headerSubtitle}>
            Monitor, update status, and reschedule patient medical service bookings
          </p>
        </div>
        ...
        <div className={serviceAppointmentsStyles.searchContainer}>
          <div className={serviceAppointmentsStyles.searchInputWrapper}>
            <div className={serviceAppointmentsStyles.searchLabel}>
              <div className={serviceAppointmentsStyles.searchIconContainer} style={{ top: "10px" }}>
          ...
          <div className={serviceAppointmentsStyles.searchInfo}>
            <span>Found {filtered.length} bookings</span>
      ...
      {loading && appointments.length === 0 ? (
        <div className="py-12 text-center text-slate-500 font-medium font-sans">
          ...
          <span>Syncing service appointments...</span>
        </div>
      ) : error ? (
        <div className="py-12 text-center text-rose-600 font-semibold font-sans">
          <AlertCircle className="w-8 h-8 mx-auto mb-2 text-rose-600" />
          <span>{error}</span>
        </div>
      ) : displayList.length === 0 ? (
        <div className="py-12 text-center text-slate-500 font-medium font-sans bg-white border border-slate-100 rounded-3xl max-w-2xl mx-auto shadow-xs">
          <span>No matching service bookings found.</span>
        </div>
      ) : (
        <div className={serviceAppointmentsStyles.cardGrid}>
          {displayList.map((a) => {
            ...
            return (
              <div key={a.id} className={serviceAppointmentsStyles.card}>
