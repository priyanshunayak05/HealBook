...
function normalizeAppointment(a) {
  ...
  const doctorImageObj =
    (a.doctorId && typeof a.doctorId === "object" && (a.doctorId.imageUrl || a.doctorId.image)) ||
    a.doctorImage?.url ||
    (typeof a.doctorImage === "string" ? a.doctorImage : null) ||
    a.doctorImageUrl ||
    null;

  const doctorImage =
    typeof doctorImageObj === "string" && doctorImageObj.trim().length > 0
      ? doctorImageObj
      : null;
...
export default function DashboardPage({ apiBase }) {
  ...
  async function updateStatusRemote(id, newStatusFrontend) {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/appointments/status/${id}`, {
        method: "PUT",
        headers,
  ...
  async function rescheduleRemote(id, newDate, newTime24) {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/appointments/reschedule/${id}`, {
        method: "PUT",
        headers,
